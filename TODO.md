...
**Updated:** February 8, 2026 (UID v1→v2 migration COMPLETED — S22 resolved. All 102 UIDs converted, zero v1 remain.)
...
| ~~S8~~ | ~~RDS CHECK constraint~~ | **LIVE VERIFIED.** The `endpoint_type` CHECK constraint was **REMOVED entirely** (not set to W,A,D,N,I,O,ND). Validation is now via dynamic trigger `trg_validate_endpoint_type` which queries `taxonomy_definition`. **Remaining hardcoded CHECKs on live:** `chk_site_uid_format` (updated Feb 8: `^sn_[0-9a-z]{12}$` for v2), `chk_web_protocol` (W2,W3), `chk_status` (planned,wip,live), `chk_persistence` (D,P), `chk_store_checkout` (Y,N), `chk_health_method` (11 pulse methods). | **RESOLVED** (corrected: trigger not CHECK) |
...
| S21 | Remaining hardcoded CHECKs should be dynamic | Live has 6 hardcoded CHECK constraints: `chk_site_uid_format` (updated Feb 8: now `^sn_[0-9a-z]{12}$` for v2), `chk_web_protocol` (W2,W3), `chk_status` (planned,wip,live), `chk_persistence` (D,P), `chk_store_checkout` (Y,N), `chk_health_method` (11 pulse methods). Only `endpoint_type`, `managed_by`, and `platform_type` are dynamic (via triggers + `taxonomy_definition`). Consider migrating the rest to dynamic validation for consistency. | **FUTURE** `LIVE` |
| ~~S22~~ | ~~`site_uid VARCHAR(12)` confirmed on live — blocks v2 UIDs~~ | **RESOLVED.** Full UID v1→v2 migration completed Feb 8, 2026. All 102 UIDs converted (94 sites + 8 agents). Schema widened VARCHAR(12)→VARCHAR(15), v1 CHECK dropped (`^SN-[0-9]{8}$`), v2 CHECK added (`^sn_[0-9a-z]{12}$`). ~490 cell updates across 10 tables on 2 RDS instances. 37 S3 folders renamed. 163 JSON entries updated in network-endpoints.json. 189 lines of dead counter code deleted from Connect Gateway (5443→5254 lines). Zero v1 UIDs remain anywhere. Mapping file: `uid_migration_mapping.json`. Full backup: `server-backups/server-full-backup-pre-uid-v2-20260208.tar.gz`. See [UID-Migration-Plan.md](UID-Migration-Plan.md) and [UID-Migration-Scope.md](UID-Migration-Scope.md). | **RESOLVED** `LOCAL+LIVE` |
...
| ~~`site_uid` width~~ | Document says VARCHAR(15) for v2 | **VARCHAR(15)** with CHECK `^sn_[0-9a-z]{12}$` (v2) — migrated Feb 8, 2026 | ~~S22: RESOLVED~~ |
...
| | | |
| **TOTAL OPEN** | **2** | 1 medium + 1 low-doc + 6 future |
| **TOTAL RESOLVED** | **38** | S1-S12, S18, S18a, S18b, S18c, S19b, S19d, S19e, S20, S22, S23, S24, S24a, Q1, Q2, Q3, D1-D10 |
...
**Live System State (Feb 8 2026 — post UID v2 migration):**
```
...
uid_registry: 102 UIDs — ALL v2 format (94 sn_, 8 sa_) + 3 su_, 2 sl_. Zero v1 UIDs (SN-/SA-) remain.
endpoint_taxonomy: site_uid VARCHAR(15), CHECK ^sn_[0-9a-z]{12}$. 82 rows, all v2.
agents: agent_uid VARCHAR(15), site_uid VARCHAR(15). 8 rows, all v2.
S3: 37 sn_ folders (0 SN- folders). Old SN- prefixes fully cleaned.
network-endpoints.json: 0 SN- references. Counter metadata removed.
Connect Gateway: 5254 lines (was 5443). Counter management code deleted. v2 generation active.
