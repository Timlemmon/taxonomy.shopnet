# ThisIsTheLaw v5.0 — Complete SND Taxonomy
...
**Supersedes:** ThisIsTheLaw3.MD (v3.5), FixingTheLaw.MD, endpoint-taxonomy-update.MD
...
## 2.6 Agent-Specific Fields (endpoint_type = 'A' only) [LIVE]

**Source:** `taxonomy_definition` table, `level = 'agent_type'` — defined in SQL file but **NOT DEPLOYED to production** (0 rows on live)
**Date:** Feb 4, 2026 (designed), NOT YET DEPLOYED
...
**Source:** `console.js` `getPulseMethod()` (line ~3854)
**Date:** Feb 7, 2026 (updated: W3→L3 fix)
...
| I | RDS | `db` |
| I | S3B | `fileserver` |
| I | default (LMB, SQS, SNS, CF, AGW) | `api` |

### Pulse Module — Full Documentation

The Pulse health check system is documented in **[42-PULSE.md](/shopnet.network-library/shopnet-deathstar/42-PULSE.md)** (v1.2, Feb 7 2026). That document is the authoritative reference for:
...
- **Pulse API routes:** `/api/v1/pulse/run`, `/api/v1/pulse/config`, `/api/v1/pulse/status`
- **Pulse controller GUI:** pulse.html, console.js pulse functions
...
**Note:** SQL file `update-taxonomy-v3.5.sql` defines ~87 rows (adds 7 agent_type + 1 inactive website_purpose 'gateway') — those 8 rows were never deployed to production.
...
### agent_type (7 rows — NOT DEPLOYED to production)

| code | name |
|------|------|
| domain | Domain Assist |
| product | Product Assist |
| content | Content Assist |
| kpi | KPI Assist |
| site | Site Assist |
| choice | Choice Assist |
| custom | Custom |

**Note:** These 7 rows are in `update-taxonomy-v3.5.sql` but were never INSERTed to production. Production `taxonomy_definition` has 0 `agent_type` rows.
