...
> **Note:** The definitive SND taxonomy source is [THIS-IS-THE-LAW v5.0](../taxonomy.shopnet/ThisIsTheLaw_v5.0_2026-02-07.MD) (Doc 47).
> This document (Doc 26) covers site_uid governance. For full namespace (5 UID types, v2 format), see Doc 47.
...
│   ════════════════════════════════════════════════                      │
│       │                                                                 │
│       ├── W:  CO, CP, WP, WW, SH, CL, S3, LM, W3                       │
│       ├── A:  CL, GP, GM, CA, OT                                        │
│       ├── D:  S3, RD, DY, GIT, TPD                                      │
│       ├── N:  W3G, API, RDH, W3S, LLM, OTH                             │
│       ├── I:  LS, EC2, RDS, CF, AGW, R53, LM, S3, DY                    │
│       └── O:  OTH                                                       │
│                                                                         │
└─────────────────────────────────────────────────────────────────────────┘
```
...
| `N` | Node | Network nodes (gateways, APIs, routing) |
| `I` | Infrastructure | AWS infrastructure components |
| `O` | Other | Other endpoint types |
...
│   ┌───────┐  ┌───────┐  ┌───────┐  ┌───────┐  ┌───────┐  ┌───────┐   │
│   │   W   │  │   A   │  │   D   │  │   N   │  │   I   │  │   O   │   │
│   │Website│  │ Agent │  │  DB   │  │ Node  │  │Infra  │  │ Other │   │
│   └───┬───┘  └───┬───┘  └───┬───┘  └───┬───┘  └───┬───┘  └───┬───┘   │
│       │          │          │          │          │          │         │
│       ▼          ▼          ▼          ▼          ▼          ▼         │
│   ┌───────┐  ┌───────┐  ┌───────┐  ┌───────┐  ┌───────┐  ┌───────┐   │
│   │Brochur│  │Claude │  │  S3   │  │ W3G   │  │  LS   │  │  OTH  │   │
│   │Portal │  │  GPT  │  │  RDS  │  │  API  │  │  EC2  │  │       │   │
│   │Console│  │Gemini │  │  DY   │  │  RDH  │  │  RDS  │  │       │   │
│   │ Store │  │Custom │  │  GIT  │  │  W3S  │  │  CF   │  │       │   │
│   └─────────┘    └─────────┘    └─────────┘    └─────────┘             │
│                                                                         │
└─────────────────────────────────────────────────────────────────────────┘
```
...
| `CA` | Custom Agent | - | Custom-built agent |
...
| `DY` | DynamoDB | AWS DynamoDB | NoSQL database |
| `GIT` | GitHub | GitHub | Repository (code/docs) |
| `TPD` | Third-Party DB | Various | Third-party database |

### N (Node)

| Code | Platform | Service | Description |
|------|----------|---------|-------------|
| `W3G` | Web3 Gateway | Freename/Blockchain | Web3 gateway bridge |
| `API` | API Gateway | AWS/External | API endpoint gateway |
| `RDH` | Redirect/Host | Various | DNS redirect or host mapping |
| `W3S` | Web3 Service | Blockchain | Web3 service endpoint |
| `LLM` | LLM Service | Various | Large language model service |
| `OTH` | Other | Various | Other node type |

### I (Infrastructure)

| Code | Platform | Service | Description |
|------|----------|---------|-------------|
| `LS` | Lightsail | AWS Lightsail | Virtual private server |
| `EC2` | EC2 | AWS EC2 | Elastic compute instance |
| `RDS` | RDS | AWS RDS | Managed database service |
| `CF` | CloudFront | AWS CloudFront | CDN distribution |
| `AGW` | API Gateway | AWS API Gateway | API management |
...
| Code | Platform | Service | Description |
|------|----------|---------|-------------|
| `OTH` | Other | Various | Other endpoint type |
...
| `endpoint_type` | CHAR(2) | W, A, D, N, I, O | YES | NO | Level 1 classification |
...
| Field | Type | Values | Required | Mutable | Description |
|-------|------|--------|----------|---------|-------------|
| `managed_by` | CHAR(1) | L, R, M, E, null | NO | YES | Management application |
...
| `E` | External | Managed by external platform (Shopify, WordPress.com, etc.) |
...
│   │ endpoint_type    CHAR(2)      NOT NULL      -- W,A,D,N,I,O   │      │
...
│                                                                         │
│   N (Node)                                                              │
│   ├── W3G Web3 Gateway         (Blockchain bridge)                      │
│   ├── API API Gateway          (API endpoint)                           │
│   ├── RDH Redirect/Host        (DNS redirect)                           │
│   ├── W3S Web3 Service         (Web3 service)                           │
│   ├── LLM LLM Service          (Language model)                         │
│   └── OTH Other                (Other node)                             │
│                                                                         │
│   I (Infrastructure)                                                    │
│   ├── LS  Lightsail            (AWS VPS)                                │
│   ├── EC2 EC2                  (AWS Compute)                            │
│   ├── RDS RDS                  (AWS Database)                           │
│   ├── CF  CloudFront           (AWS CDN)                                │
│   ├── AGW API Gateway          (AWS API Mgmt)                           │
│   ├── R53 Route53              (AWS DNS)                                │
│   ├── LM  Lambda               (AWS Serverless)                         │
│   ├── S3  S3                   (AWS Storage)                            │
│   └── DY  DynamoDB             (AWS NoSQL)                              │
│                                                                         │
│   O (Other)                                                             │
│   └── OTH Other                (Other endpoint)                         │
│                                                                         │
└─────────────────────────────────────────────────────────────────────────┘
```
...
site_uid (THE LAW)
    ...
    │       │
    │       ├── W (Website)
    │       │       ├── platform_type: CO, CP, WP, WW, SH, CL, S3, LM, W3
    │       │       ├── managed_by: L, R, M, E
    │       │       ├── persistence: D, P
    ...
    │       │
    │       ├── A (Agent)
    │       │       └── platform_type: CL, GP, GM, CA, OT
    │       │
    │       ├── D (Database)
    │       │       └── platform_type: S3, RD, DY, GIT, TPD
    │       │
    │       ├── N (Node)
    │       │       └── platform_type: W3G, API, RDH, W3S, LLM, OTH
    │       │
    │       ├── I (Infrastructure)
    │       │       └── platform_type: LS, EC2, RDS, CF, AGW, R53, LM, S3, DY
    │       │
    │       └── O (Other)
    │               └── platform_type: OTH
    │
...
└─────────────────────────────────────────────────────────────────────────────────────────────────────┘
  ...
  │   • assigned_at → When site_uid was issued                                         │
  │   • endpoint_type → W/A/D/N/I/O classification                                     │
  │                                                                                    │
  └────────────────────────────────────────────────────────────────────────────────────┘
```
...
│  │ endpoint_type     │ endpoint_taxonomy      │ W/A/D/N/I/O - set by Radius GUI                     ││
...
│  │ managed_by        │ endpoint_taxonomy      │ L/R/M/E (websites only)                             ││
...
│   │   Scope: All endpoints (W, A, D, N, I, O)                                                    │   │
...
CREATE TABLE IF NOT EXISTS endpoint_taxonomy (
    ...
    -- Constraints
    -- NOTE: Static CHECK constraints replaced by dynamic validation triggers on production (v3.5.1, Feb 7 2026)
    -- Triggers: validate_endpoint_type, validate_managed_by, validate_platform_type
    -- These read valid values from taxonomy_definition table instead of hardcoded CHECKs
...
| Endpoint type [W] | endpoint_type | endpoint_taxonomy - RDS (W/A/D/N/I/O) |
| Platform type [CO] | platform_type | endpoint_taxonomy - RDS |
| Managed by [L] | managed_by | endpoint_taxonomy - RDS (L/R/M/E) |
...
| Type | endpoint_type | endpoint_taxonomy | W, A, D, N, I, O |
| Platform | platform_type | endpoint_taxonomy | CO, CP, WP, WW, SH, CL, S3, LM, W3, GP, GM, CA, OT, RD, DY, GIT, TPD, W3G, API, RDH, W3S, LLM, OTH, LS, EC2, RDS, CF, AGW, R53 |
...
| Managed | managed_by | endpoint_taxonomy | L (Lambda), R (Radius), M (Manual), E (External) - websites only |
...
|---------------|---------------------------|
| W (Website) | CO (Custom On-Demand), CP (Custom Persistent), WP (WordPress), WW (WooCommerce), SH (Shopify), CL (CloudFront), S3 (S3 Static), LM (Lambda), W3 (Web3 Site) |
| A (Agent) | CL (Claude), GP (GPT), GM (Gemini), CA (Custom Agent), OT (Other) |
| D (Database) | S3 (S3 Bucket), RD (RDS), DY (DynamoDB), GIT (GitHub Repo), TPD (Third-Party DB) |
| N (Node) | W3G (Web3 Gateway), API (API Gateway), RDH (Redirect/Host), W3S (Web3 Service), LLM (LLM Service), OTH (Other) |
| I (Infrastructure) | LS (Lightsail), EC2, RDS, CF (CloudFront), AGW (API Gateway), R53 (Route53), LM (Lambda), S3, DY (DynamoDB) |
| O (Other) | OTH (Other) |
