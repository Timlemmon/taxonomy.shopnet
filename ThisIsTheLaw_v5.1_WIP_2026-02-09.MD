# ThisIsTheLaw v5.1 WIP — Complete SND Taxonomy
**Date:** February 9, 2026
**Version:** v5.1 WIP (Work In Progress)
**Authority:** TJL
**Authors:** TJL, Claude (Sonnet 4.5)
**Status:** WORK IN PROGRESS — based on v5.0, adding unified taxonomy system for all UID types
...
**Based On:** ThisIsTheLaw_v5.0_2026-02-07.MD

**Supersedes (when finalized):** ThisIsTheLaw_v5.0_2026-02-07.MD
...
**Constraint:** `endpoint_taxonomy.chk_endpoint_type CHECK (endpoint_type IN ('W','A','D','N','I','U','O','ND'))` — includes ND for backward compatibility, U added Feb 9 2026
**Date:** Feb 4-7, 2026
...
| `A` | Agent Platform | Infrastructure hosting AI agents (not the agent itself - see sa_ for agent instances) | TRUE |
...
| `U` | User Device | End-user client devices (phones, tablets, laptops, desktops, watches, TVs) | TRUE — **Added Feb 9 2026** |
...
### A (Agent Platform) — 5 types [LIVE]

**Source:** `taxonomy_definition` table, `parent_code = 'A'` — `shopnet_sites` RDS

**IMPORTANT DISTINCTION:**
- **endpoint_type='A'** = Agent PLATFORM (infrastructure that hosts agents)
  - Example: `sn_0ta4kc0zxldu` = Claude Agent Platform (infrastructure)
  - Stored in: `shopnet_sites.endpoint_taxonomy`
  - Purpose: Infrastructure/endpoint that runs agent software
- **Agent Instance (sa_ UID)** = The actual AI agent running ON the platform
  - Example: `sa_0ta4kc147c51` = Product Assist (agent instance)
  - Stored in: `shopnet_assist.agents`
  - Links to platform via `site_uid` column

**Relationship:** One Agent Platform (sn_ with endpoint_type='A') can host one or more Agent Instances (sa_ UIDs).

**Platform Types (Infrastructure):**

| Code | Name | Description | In Wizard |
|------|------|-------------|-----------|
| `CL` | Claude Agent | Anthropic Claude-based agent | YES |
| `GP` | GPT Agent | OpenAI GPT-based agent | YES |
| `GM` | Gemini Agent | Google Gemini-based agent | YES — added to fallback Feb 7 |
| `CA` | Custom Agent | Custom AI agent implementation | YES |
| `OT` | Other Agent | Other AI agent | YES — added to fallback Feb 7 |
...
### U (User Device) — 7 platform types [WIP]

**Source:** `taxonomy_law` table, `parent_code = 'U'` — `shopnet_connect` RDS
**Added:** February 9, 2026
**Status:** WIP — Database entries created, not yet in production use
**Pulse Method:** `none` (pending client app) — User devices cannot be checked from outside without app sending heartbeats

**Platform Types (Operating Systems):**

| Code | Name | Description | L3 Form Factors |
|------|------|-------------|-----------------|
| `iOS` | Apple iOS | Apple iOS ecosystem | phone, tablet, watch, tv |
| `Android` | Google Android | Android ecosystem | phone, tablet, watch, tv |
| `Windows` | Microsoft Windows | Windows OS | laptop, desktop, tablet |
| `macOS` | Apple macOS | Apple desktop/laptop OS | laptop, desktop |
| `Linux` | Linux | Linux distributions | laptop, desktop |
| `ChromeOS` | Chrome OS | Google Chrome OS | laptop |
| `Other` | Other OS | Other operating systems | other |
...
**Example UIDs:**
```
sn_abc123 → U → iOS → phone (iPhone 15)
sn_def456 → U → macOS → laptop (MacBook Pro)
sn_ghi789 → U → Android → tablet (Samsung Tab)
sn_jkl012 → U → Windows → desktop (Dell Desktop)
```

**Pulse Monitoring:**
User devices are client-side (behind NAT/firewalls by design) and cannot be checked from outside. Future implementation will require a client app to send periodic heartbeats to the server for device status tracking.

**Note:** Replaces DEV platform_type which was previously under Infrastructure (I). User devices are fundamentally different from infrastructure (backend systems) and warranted their own endpoint_type.
...
| `endpoint_type` | CHAR(2) | W, A, D, N, I, U, O | YES | **NO** |
...
**Note:** This is NOT the same as `shopnet_assist.agents.agent_type` which classifies agents themselves as `internal` or `client` (CHECK constrained). See S18c in TODO.md for full investigation.
...
| A | (all) | `agent` | POST test prompt to LLM |
...
**Source:** Document 26: THIS IS THE LAW v4.0, Part VI — `/shopnet-deathstar/26-THIS-IS-THE-LAW.md`
**Source:** `DATABASE-DESIGN.md` — `/assist.shopnet/docs/DATABASE-DESIGN.md` (Feb 5-6, 2026)
**Date:** February 5-6, 2026
**Authority:** TJL
**Implementation Status:** [LIVE] — Database `shopnet_assist` created on Instance 1. All 9 tables created. 12 seed rules inserted. 8 agents registered (SA-00000001 through SA-00000008). **No backend code yet** (see S14 in TODO).

**Note:** shopnet_assist has NO `taxonomy_definition` equivalent table. Agent types and skills are constrained by MySQL-style ENUM types in DDL (agent_type, status, model_tier) and unconstrained VARCHAR (agent_skill). See TODO S16 for future work.

---

## 3.1 Two-Level Identity System [LIVE]

**Source:** Document 26, Part VI.1 (lines 2975-3000)
**Date:** February 5-6, 2026

Every AI agent receives BOTH a `site_uid` (as a network endpoint) AND an `agent_uid` (as an operational agent). This is THE LAW.
...
Level 1 (Instance):  agent_uid  SA-XXXXXXXX
  ...
  - REQUIRES site_uid first
```

---

## 3.2 agent_uid Format [LIVE]
...
## 3.3 Issuance Process — Two-Step Registration [LIVE]
...
## 3.5 Agent Subtypes [LIVE]

**Source:** `DATABASE-DESIGN.md`, Section 2 (lines 56-57)

Agent type system in agents table:
- `agent_type`: ENUM('internal', 'client')
- `agent_skill`: VARCHAR(50) — values: 'code', 'chat', 'content', 'data', 'documents', 'promotion'

---

## 3.6 Level Separation Rules [LIVE]
...
5. Both UIDs are generated ONLY by Connect Gateway

---

## 3.7 Registered Agent Platforms [LIVE]

**Source:** `DATABASE-DESIGN.md`, Section 2 seed data
**Date:** February 6, 2026
...
| 12 | `DATABASE-DESIGN.md` | assist.shopnet | Agent schema (9 tables) | Feb 5-6 | **CURRENT** |
...
| **Agent registry** | shopnet_assist | agents (+ 8 more tables) | — (no backend engine) | Schema + seed data only |
...
| **Agent backend** | `DATABASE-DESIGN.md` (9 tables) | None | Backend engine not built (S14). Prompt pipeline, self-policing — all designed, none coded |
...
| **taxonomy_definition for assist** | Not designed | No equivalent lookup table (S16) |
| **taxonomy_definition for registry** | Not designed | No equivalent lookup table (S17) |
| **assist.shopnet backend** | Designed only | DATABASE-DESIGN.md describes 9 tables + pipeline (S14) |
...
const ENDPOINT_TYPES = {
    'W': 'Website', 'A': 'Agent', 'D': 'Database', 'ND': 'Node'
};
